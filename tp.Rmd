...
plot(x=evolution$dates, y=evolution$nombres, type="l")
```
Les résultats montrent que plus on augmente la fréquence d'arrivées, moins la file d'attente a de chances de se vider.
...
# par(new = TRUE)
xseq<-seq(0, n, 0.1)
plot(xseq, dnorm(xseq, n*p, n*p*(1-p)), col='blue', type='l')
```
# Question Bonus 2

Pour le même résultat, le temps d'exécution moyen de la simulation par rejet est 10 fois plus grand que celui de la simulation par inversion.
